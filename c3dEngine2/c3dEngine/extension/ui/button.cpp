...
bool Cbutton::init(float width,float height,string imagePath_up,string imagePath_dn,string imagePath_text){
	...
	m_height=height;
	//--texture
	m_textureUp=Cc3dTextureCache::sharedTextureCache()->addImage(imagePath_up,GL_CLAMP_TO_EDGE,GL_CLAMP_TO_EDGE);
	m_textureUp->retain();
	m_textureDn=Cc3dTextureCache::sharedTextureCache()->addImage(imagePath_dn,GL_CLAMP_TO_EDGE,GL_CLAMP_TO_EDGE);
	m_textureDn->retain();

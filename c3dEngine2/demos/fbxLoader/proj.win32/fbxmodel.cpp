...
//系统自带语句				FBXSDK_printf("\n");
	...
	void Cmodelfbx::makeSubMeshSetForThisNode(FbxNode* pNode,int userDataIndex){
		...
		CidSubMeshSet*pIdSubMeshSet=new CidSubMeshSet();
		//将pIdSubMeshSet添加到lMesh节点
		lMesh->SetUserDataPtr(userDataIndex,pIdSubMeshSet);
		//制作pIdSubMeshSet
		//每个材质对应一个subMesh
		{
			...
			//为pIdSubMeshSet生成元素
			//pIdSubMeshSet应包含maxMaterialIndex+1个idSubMesh，分别与maxMaterialIndex+1个Material对应
			//对于那些没有Material相对应的多边形，将它加入到0号材质对应的subMesh中
			pIdSubMeshSet->idSubMeshList.resize(maxMaterialIndex+1);
			//为pIdSubMeshSet->idSubMeshList各元素的polygenIndexList和vIDList预留空间
			const int triangleCount = lMesh->GetPolygonCount();
			const int controlPointCount = lMesh->GetControlPointsCount();
			{
				int nIdSubMesh=pIdSubMeshSet->idSubMeshList.size();
				for(int i=0;i<nIdSubMesh;i++){
					pIdSubMeshSet->idSubMeshList[i].polygenIndexList.reserve(triangleCount);
					pIdSubMeshSet->idSubMeshList[i].vIDList.reserve(controlPointCount);
				}
			}
			//遍历各多边形，计算它们的材质号（subMesh号）、纹理号
			
			for (int i = 0; i < triangleCount; i++)
			{
				//当前多边形号：i
				//获得材质号(subMesh号)：lMaterialIndex
				...
				//获得纹理：pTextureName
				//每个Matrial只对应一个纹理，所以很可能本纹理已经通过相同Matrial的其它多边形得到过了，
				//所以在获得纹理之前先判断一下是否已得到过，省去再次获得的麻烦
				if(pIdSubMeshSet->idSubMeshList[lMaterialIndex].texture==0){
					GLuint texture=0;
					string texPath;
					{
						//根据lMaterialIndex获得纹理
						//由材质索引获得材质节点，再由材质节点获得纹理节点，而纹理信息当初加载时已经以用户数据的形式记录在纹理节点上了。
						const FbxSurfaceMaterial * lMaterial = pNode->GetMaterial(lMaterialIndex);//当前多边形材质
						...
						if (lProperty.IsValid())
						{
							...
							if (lTextureCount)
							{
								...
								{
									texture = *(static_cast<GLuint *>(lTexture->GetUserDataPtr(userDataIndex_texture)));
									texPath=*(static_cast<string*>(lTexture->GetUserDataPtr(userDataIndex_texPath)));
								}
							}
						}
					}//得到pTexureName
					if(texture==0){
				//	cout<<"warning:模型中存在无贴图材质!"<<endl;
						//不用下面两句，而是直接令无贴图的材质的贴图为0（模型转化成mesh时贴图为0的面将被忽略掉）
					/*	{
						texture=tex_white;
						cout<<"使用白色纹理代替"<<endl;
						}*/
					}
					pIdSubMeshSet->idSubMeshList[lMaterialIndex].texture=texture;
					pIdSubMeshSet->idSubMeshList[lMaterialIndex].texPath=texPath;
					
				}
				//将本多边形索引加入到pIdSubMeshSet->idSubMeshList[lMaterialIndex]
				pIdSubMeshSet->idSubMeshList[lMaterialIndex].polygenIndexList.push_back(i);
				//获得此多边形的三个顶点索引，并加入到pIdSubMeshSet->idSubMeshList[lMaterialIndex]
				int ID0=lMesh->GetPolygonVertex(i,0);
				int ID1=lMesh->GetPolygonVertex(i,1);
				int ID2=lMesh->GetPolygonVertex(i,2);
				pIdSubMeshSet->idSubMeshList[lMaterialIndex].vIDList.push_back(ID0);
				pIdSubMeshSet->idSubMeshList[lMaterialIndex].vIDList.push_back(ID1);
				pIdSubMeshSet->idSubMeshList[lMaterialIndex].vIDList.push_back(ID2);
			}
		}
...
void Cmodelfbx::GetSmoothing(FbxManager* pSdkManager, FbxNode* pNode, bool pCompute, bool pConvertToSmoothingGroup)
{
	...
	void Cmodelfbx::destroyAllUserData(){
		//清除用户保存的纹理句柄和纹理索引
		{
			...
			for (int lTextureIndex = 0; lTextureIndex < lTextureCount; ++lTextureIndex)
			{
				...
				if(lFileTexture && lFileTexture->GetUserDataPtr(userDataIndex_texPath)){
					string* texPath =(static_cast<string *>(lFileTexture->GetUserDataPtr(userDataIndex_texPath)));
					delete texPath;
				}
			}
		}
	...
	void Cmodelfbx::clearUserDataRecursive(FbxNode* pNode)
	{
		...
		{	
			...
			if(pIdSubMeshSet!=NULL)delete pIdSubMeshSet;
			//销毁UV
...
//				cout<<filename<<endl;
				//以filename生成纹理
				if(CTextureManager::sharedTextureManager()->CreateTexture((char*)filename.c_str())){
					GLuint * texture = new GLuint(CTextureManager::sharedTextureManager()->getLastTex());
					string*texPath=new string();
					*texPath = filename;
					//将此纹理的显存句柄以用户数据的形式保存回到lFileTexture(即lTexture)
                    lFileTexture->SetUserDataPtr(userDataIndex_texture,texture);
					//将此纹理的路径以用户数据的形式保存回到lFileTexture(即lTexture)
					lFileTexture->SetUserDataPtr(userDataIndex_texPath,texPath);
				}else{
					cout<<"warning: create texture failed! "<<endl;
					system("pause");
					exit(0);
				}
		...
		//检查是否有重复纹理路径
		{
			...
			if(lTextureCount<=1){//如果纹理数小于等于1
				...
				for (int i = 0; i < lTextureCount; i++)
			    {
					...
					string texPath_i=*(static_cast<string*>(lTexture->GetUserDataPtr(userDataIndex_texPath)));
					//用texPath_i与其之后的各path_j比较，如果有相同者，则说明有重复
					for(int j=i+1;j<lTextureCount;j++){
						FbxTexture * lTexture = lScene->GetTexture(j);
						string texPath_j=*(static_cast<string*>(lTexture->GetUserDataPtr(userDataIndex_texPath)));
